...
function renderSalesImportReview(){const p=pendingSalesImport;if(!p)return;const counts={matched:0,suggested:0,unmatched:0,excluded:0};p.rows.forEach(x=>counts[x.status]++);const importQty=p.rows.filter(x=>!x.excluded&&x.lineId).reduce((s,x)=>s+x.qty,0),excludedQty=p.rows.filter(x=>x.excluded).reduce((s,x)=>s+x.qty,0);document.getElementById('salesImportSummary').innerHTML=`${p.duplicateWarning?'<div class="sales-import-warning"><strong>Possible duplicate:</strong> this same file has already been imported for this property and period.</div>':''}<div class="sales-import-summary"><div class="recipe-meta-card"><strong>Detected Format</strong>${escapeHtml(p.format)}</div><div class="recipe-meta-card"><strong>Report Property</strong>${escapeHtml(p.property||'Not detected')}</div><div class="recipe-meta-card"><strong>Report Dates</strong>${escapeHtml(p.start||'?')} to ${escapeHtml(p.end||'?')}</div><div class="recipe-meta-card"><strong>Report Qty</strong>${p.reportQty||p.fullQty}</div><div class="recipe-meta-card"><strong>Mapped Qty</strong>${importQty}</div><div class="recipe-meta-card"><strong>Excluded Qty</strong>${excludedQty}</div></div>`;
 const rows=p.rows.map((x,i)=>`<tr class="${x.excluded?'sales-import-row-excluded':!x.lineId?'sales-import-row-unmatched':''}"><td>${escapeHtml(x.rvc||'')}</td><td>${escapeHtml(x.category)}</td><td>${x.posId?`<span style="color:#657786">${escapeHtml(x.posId)}</span> `:''}<strong>${escapeHtml(x.name)}</strong></td><td>${x.qty}</td><td>${formatCurrency(x.sales||0)}</td><td><span class="sales-import-status ${x.status}">${x.status==='matched'?'Matched':x.status==='suggested'?`Suggested ${Math.round((x.score||0)*100)}%`:x.status==='excluded'?'Excluded':'Needs Review'}</span></td><td><select onchange="setSalesImportMatch(${i},this.value)">${menuLineOptions(x.lineId)}</select></td><td><div class="sales-import-actions"><button class="mini-action-btn" onclick="createSalesImportDraft(${i})">Create Draft</button><button class="mini-action-btn" onclick="toggleSalesImportExclude(${i},false)">${x.excluded?'Include':'Exclude Once'}</button><button class="mini-action-btn" onclick="toggleSalesImportExclude(${i},true)">${x.alwaysExcluded?'Remove Rule':'Always Exclude'}</button></div></td></tr>`).join('');
 ...
 const unresolved=p.rows.filter(x=>!x.excluded&&!x.lineId).length;document.getElementById('applySalesImportBtn').disabled=unresolved>0||!activePeriodRecord()||activePeriodRecord().finalized;document.getElementById('applySalesImportBtn').textContent=unresolved?`Resolve ${unresolved} Item(s) Before Import`:'Apply Import';}
