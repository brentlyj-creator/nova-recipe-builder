...
let menuMixDirty=false, engineeringCategories=new Set(), engineeringRows=[];
...
function engineeringSource(){const r=activePeriodRecord(),menu=document.getElementById('engineeringMenuSelector')?.value||'ALL';return(r?.menuMixSnapshot||[]).filter(x=>menu==='ALL'||x.menuId===menu).map(x=>{const qty=Number(x.soldQty)||0,price=Number(x.price)||0,cost=Number(x.cost)||0,cm=price-cost;return{...x,qty,price,cost,cm,sales:qty*price,totalCost:qty*cost,totalContribution:qty*cm};});}
...
function setEngCats(mode){const rows=engineeringSource(),cats=[...new Set(rows.map(x=>x.categoryName||'Uncategorized'))].sort();engineeringCategories=new Set(mode==='all'?cats:mode==='sales'?cats.filter(c=>rows.some(x=>(x.categoryName||'Uncategorized')===c&&x.qty>0)):[]);renderEngineering();}
function toggleEngCat(cat,on){on?engineeringCategories.add(cat):engineeringCategories.delete(cat);renderEngineering();}
...
async function parseMicrosPdf(file){
 ...
 for(const line of lines){let m=line.match(/^\d+\s*-\s*(Restaurant|Lounge|.+)$/i);if(m){rvc=m[1].trim();continue;}m=line.match(/^Total\s+(.+?)\s+0\s+[\d,.]+\s+/i);if(m){flush(m[1].trim());continue;}m=line.match(/^Grand Total\s+([\d,.]+).*?([\d,.]+)\s+0\.00\s+([\d,.]+)/i);if(m){reportQty=Number(m[1].replace(/,/g,''))||0;reportSales=Number(m[3].replace(/,/g,''))||Number(m[2].replace(/,/g,''))||0;continue;}
  m=line.match(/^(\d{3,7})\s+(.+?)\s+(SGL|REG\$|HHR\$)\s+(.+)$/i);if(!m)continue;const tail=m[4],prePct=(tail.match(/^(.+?)\s+\d+(?:\.\d+)?%/)||[])[1];if(!prePct)continue;const nums=prePct.match(/-?\d+(?:\.\d+)?/g)||[];if(!nums.length)continue;const qty=Number(nums[nums.length-1]);const price=nums.length>1?Number(nums[0]):0;if(!Number.isFinite(qty)||qty===0)continue;buffer.push({pos:'micros',category:'Pending Group',name:m[2].trim(),qty,sales:price*qty,price,rvc,posId:m[1],excluded:false});}
