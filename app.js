...
function updateBulkCount(){const count=document.querySelectorAll('.ingredient-pick-cb:checked').length,e=document.getElementById('ingredientBulkCount'),bar=document.getElementById('ingredientBulkActions'),btn=document.getElementById('addSelectedIngredientsBtn');if(e)e.textContent=`${count} selected`;if(btn)btn.textContent=`Add Selected Ingredients (${count})`;if(bar)bar.style.display=count?'flex':'none';}
